...
static unsigned long long h1(string name, unsigned long long m)      //convert into long long
{
    ...
    void Print(FILE* file)      //print into file
    {
        ...
        for(long long i=0; i<maxSize; i++)
        {
            ...
            while(pt!=NULL)
            {
                ...
                if(pt->isFun()=="decFunc")
                    fprintf(file,"<%s, FUNCTION, %s> ",pt->getName().c_str(),pt->getType().c_str());
                else if(pt->isFun()=="defFunc")
                    fprintf(file,"<%s, FUNCTION, %s> ",pt->getName().c_str(),pt->getType().c_str());
                else if(pt->isFun()=="errFunc")
                    fprintf(file,"<%s, FUNCTION, %s> ",pt->getName().c_str(),pt->getType().c_str());
                else if(pt->isFun()=="array")
                    fprintf(file,"<%s, ARRAY, %s> ",pt->getName().c_str(),pt->getType().c_str());
                else
